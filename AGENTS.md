...
| `events` | id, groupId → groups, title, eventType, startTime, endTime, callTime, timezone, location, createdFromRequestId, reminderSentAt, confirmationReminderSentAt | Links back to availability request if created from one. `callTime` is nullable (only for shows — performer arrival time). `timezone` is IANA timezone string (NOT NULL, default `'America/Los_Angeles'`) — captured at event creation for group-facing displays (Discord webhooks). `reminderSentAt` tracks when reminder email was sent. `confirmationReminderSentAt` tracks when 48-hour confirmation reminder was sent to pending attendees. |
...
| `ENABLE_REMINDERS` | Optional | Set to `"true"` to enable the cron job that sends reminder emails 24h before events |

...
| `events` | id, groupId → groups, title, eventType, startTime, endTime, callTime, location, createdFromRequestId, reminderSentAt, confirmationReminderSentAt | Links back to availability request if created from one. `callTime` is nullable (only for shows — performer arrival time). `reminderSentAt` tracks when reminder email was sent. `confirmationReminderSentAt` tracks when 48-hour confirmation reminder was sent to pending attendees. |
